...
class KDTree
{
  ...
  public:
    ...
    Point<Dim> findNearestHelper(int curDim, const Point<Dim> &query, int left, int right, const Point<Dim> &currentBest) const;
  ...
  private:
     ...
     */
     ...
     int partition(vector<Point<Dim>> & Points, int left, int right, int pivotIndex, int d);
	  ...
	  KDTreeNode * makeTree(vector<Point<Dim>>& Points, int left, int right, int d);
    void clear();
