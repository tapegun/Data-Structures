...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (int x = 0; x < (int)image.width(); x++) {
    for (int y = 0; y < (int)image.height(); y++) {
      HSLAPixel & pixel = image.getPixel(x, y);
      double xdist = std::abs(x-centerX);
      double ydist = std::abs(y-centerY);
      if (std::sqrt((xdist*xdist) + (ydist*ydist)) >= 160.0){
        pixel.l = .2*pixel.l;
      }
      else{pixel.l = (pixel.l *(1 - (std::sqrt((xdist*xdist) + (ydist*ydist)))*.005));}
...
PNG watermark(PNG firstImage, PNG secondImage) {
  ...
  for (int x = 0; x < (int)secondImage.width(); x++) {
        for (int y = 0; y < (int)secondImage.height(); y++) {
