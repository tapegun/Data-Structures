...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    int addLoc = hashes::secondary_hash(key, size);
	while (table[location] != NULL)
	{
		location = (location+addLoc)%size;       //finding next spot
	}
    table[location] = new std::pair<K, V>(key, value);
	should_probe[location] = true;
}
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    int addLoc = hashes::secondary_hash(key, size);
    int start = location;
    while (should_probe[location]) {
        if (table[location] != NULL && table[location]->first == key)
            return location;
        location = (location + addLoc) % size;
        if (location == start)
            break;
    }
    return -1;
