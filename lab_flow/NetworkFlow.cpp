  ...
  while(findAugmentingPath(source_, sink_, path)){
    ...
    for(size_t i = 0; i < path.size() - 1; i++){
      if(flow_.edgeExists(path[i], path[i + 1])){
        line = flow_.getEdge(path[i], path[i + 1]);
        flow_.setEdgeWeight(path[i], path[i + 1], line.getWeight() + capacity);
      }
      else{
        line = flow_.getEdge(path[i + 1], path[i]);
        flow_.setEdgeWeight(path[i + 1], path[i], line.getWeight() - capacity);
      }
      line = residual_.getEdge(path[i + 1], path[i]);
      residual_.setEdgeWeight(path[i + 1], path[i], line.getWeight() + capacity);
      line = residual_.getEdge(path[i], path[i + 1]);
      residual_.setEdgeWeight(path[i], path[i + 1], line.getWeight() - capacity);
    }
  }
  return flow_;
}
