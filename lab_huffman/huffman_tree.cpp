                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode* merge = NULL;
    if(!singleQueue.empty()){single = singleQueue.front();}
    if(!mergeQueue.empty()){merge = mergeQueue.front();}

    if (single == NULL && merge == NULL)
	{
		return NULL;
	}
    if (merge == NULL)
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	{
        TreeNode* leftBranch = removeSmallest(singleQueue, mergeQueue);
		TreeNode* rightBranch = removeSmallest(singleQueue, mergeQueue);
        TreeNode* sum = new TreeNode(leftBranch->freq.getFrequency() + rightBranch->freq.getFrequency());
        sum->right = rightBranch;
		sum->left = leftBranch;
        mergeQueue.push(sum);
    }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        //  else{ 
        //      std::cout<<"encoding screwed up somewhere" << std::endl;
        //  }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits() == 0){return NULL;}
		...
		TreeNode * outer = new TreeNode(freq);
		return outer;
	}
	else
	{
